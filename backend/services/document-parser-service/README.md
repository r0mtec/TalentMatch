...
Дополнительно сервис делает ограниченное regex-извлечение простых анкетных полей из текста резюме: `grade`, `location`, `citizenship`, `languages` и текстовых секций. Это вспомогательная эвристика, а не полноценный анализ резюме.
...
- возвращает `candidate_id`, `status`, `plain_text`, `sections`, `warnings`;
...
- не принимает regex-извлечение анкетных полей за обязательную истину: если поле не найдено, оно остается `null`.
...
После успешного парсинга `plain_text` передается в `skill-recognition-service`, а найденные `sections.grade`, `sections.location`, `sections.citizenship` и `sections.languages` сохраняются в карточку кандидата, если эти поля не были переданы вручную при загрузке.
