...
| Справочник технологий | `GET/POST/PATCH/DELETE /technologies`, `POST /technologies/{id}/synonyms`, `PATCH/DELETE /technology-synonyms/{id}`, `GET /unrecognized-terms`, `POST /unrecognized-terms/{id}/promote` | подключено; синонимы редактируются chips, в таблице отображаются первые 3 + `+N`, нераспознанные термины можно превратить в технологию | real API + local fallback |
